...
n=100
data = np.abs(rescaled_values[np.ix_(pot_idx[n],err_names_idx)])*100
xmin = -100
xmax =  100
ymin = -11 
ymax = 0
...
for i,v in enumerate(sr.err_names):
    lc_pct_err.append(100*np.abs(qoi_ref_lc[v]/qoi_ref_dft[v.strip('.err')]))
    bg1_pct_err.append(100*np.abs(qoi_ref_bg1[v]/qoi_ref_dft[v.strip('.err')]))
    bg2_pct_err.append(100*np.abs(qoi_ref_bg2[v]/qoi_ref_dft[v.strip('.err')]))

latex_names = []
for v in sr.err_names:
    latex_names.append(qoi_ref_latex[v.strip('.err')])
...
print(len(theta),data.shape)
print(len(theta),len(lc_pct_err))
print(len(theta),len(bg1_pct_err))
print(len(theta),len(bg2_pct_err))
ax.plot(theta,data.T,color='k')
ax.plot(theta,lc_pct_err,color='b')
ax.plot(theta,bg1_pct_err,color='r')
ax.plot(theta,bg2_pct_err,color='g')
ax.set_rgrids([20,40,60,80,100])
ax.set_varlabels(latex_names)
ax.legend(handles=[mpatches.Patch(color='b',label='LC'),
                   mpatches.Patch(color='r',label='BG+2.0'),
                   mpatches.Patch(color='g',label='BG+1.7')],
          fontsize=10,
          bbox_to_anchor=(1.1, 1),
          loc='best')
fig.savefig('fig_3radar.png',png=800)
