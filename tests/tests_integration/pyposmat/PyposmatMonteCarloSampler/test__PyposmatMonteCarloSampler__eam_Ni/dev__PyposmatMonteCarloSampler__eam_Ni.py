...
for p in engine.free_parameter_names:
    if engine.parameter_distribution_definition[p][0] == 'uniform':
        a = engine.parameter_distribution_definition[p][1]['a']
        b = engine.parameter_distribution_definition[p][1]['b']
        _loc = a
        _scale = b-a
...
for i_sample in range(_n_samples):
    ...
    _constrained_parameters = [
            p for p in _parameters if p not in engine.free_parameter_names]
