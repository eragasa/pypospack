...
def func_zopemishin_eos(rho,E0,r0,alpha,beta):
    """ functio for the Zope Mishin equation of state
    Args:
        rho(np.ndarray): either a float or an array of electron density values
        E0(float): a parameter
        r0(float): another parameter
        alpha(float): yet another parameter
        beta(float): the final parameter
    Returns:
        np.ndarray: the array of values of the embedding function
    """
    F = - E0*(
            1
            + alpha * x,
            + beta * alpha**3 * (x**3) * (2*x+3) / ((x+1)**2)
        ) * np.exp(-alpha*x)
