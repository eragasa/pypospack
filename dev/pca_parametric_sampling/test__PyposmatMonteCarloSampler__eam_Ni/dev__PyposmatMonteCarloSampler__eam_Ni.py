...
class PcaSampler(PyposmatMonteCarloSampler):
...
###############################################################################
        elif _sampling_type == 'pca':
            _pca_info = None
            self.run_pca_sampling(n_samples,_pca_info)
        else:
            raise ValueError('unknown sampling option:{}'.format(
                _sampling_type))
...
###############################################################################
    ...
    def run_pca_sampling(self,n_samples,pca_info):
        print('pca_info:{}'.format(pca_info))
        print('i am lazy, so i do nothing')
