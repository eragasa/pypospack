           ...
           for j in range(lt_n_rows):
               ...
               if freq_kpoint_key == lt_kpoint_key:
                   for k in range(3*self.natoms):
                       ...
                       # we need the index associated with the phonon lifetime
...
if __name__ == "__main__":
    ...
    bte_data = PhontsBteData(natoms=8,directory=phonts_sim_dir)
    bte_data.read()
	
	
    for k in bte_data.ph_lt_file.temp:
    	bte_data.build_data_at_temp(temp=k)
    
    ph_freq = bte_data.data[k][:,4]
    ph_lt = bte_data.data[k][:,5]
    figure = plt.figure()
    freq_lt_plot = figure.add_subplot(111)
    freq_lt_plot.scatter(ph_freq,ph_lt)
    freq_lt_title = plt.title('Phonon liftime as a function of frequency', fontname = 'Times New Roman')
    freq_lt_xlabel = plt.xlabel('Frequency (THz)', fontname = 'Times New Roman')
    freq_lt_ylabel = plt.ylabel(r'$\tau^{-1}\/(ps^{-1)}$ ', fontname = 'Times New Roman')

    #set axis here
   # if xlim is not None:
   #     freq_lt_plot.set_xlim(xlim)
   # if xlim is not None:
   #     freq_lt_plot.set_ylim(ylim)
    figure.savefig('freq_vs_lt.png')
    plt.close 
